...
pub struct CodeEditor {
    ...
    /// Optional custom syntect theme for token colors. When `None`, falls back
    /// to the crate default (`base16-ocean.dark`). Set via `set_syntax_theme`.
    pub(crate) syntax_theme: Option<Theme>,
...
impl CodeEditor {
    ...
    /// Overrides the syntect theme used for syntax-highlight token colors.
    ///
    /// By default the editor uses `base16-ocean.dark`. Call this to make token
    /// colors match a host application's own palette (the editor's chrome is
    /// still controlled separately via [`set_theme`]). Changing the theme
    /// invalidates the highlight + content caches so token colors are rebaked.
    pub fn set_syntax_theme(&mut self, theme: Theme) {
        self.syntax_theme = Some(theme);
        self.content_cache.clear();
        self.overlay_cache.clear();
        self.highlight_cache.borrow_mut().take();
    }

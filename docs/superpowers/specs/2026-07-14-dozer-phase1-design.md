...
3. **验收闭环雏形**（软件项目模板）：目标/验收标准 → agent 交付 → 验收（通过/打回）→ 演进史。关口松紧由 dogfooding 调；治理结构是可配置领域模板，非硬编码姿态。**P1f 达成（2026-07-19，纵向薄片：goal.md 定标 + 交付横幅（回合毕且有变更）+ 验收 tab（标准勾选/变更文件清单/意见/通过沉淀/打回注回）+ git ref 沉淀 refs/dozer/accepted/<n> + dozerd SQLite 验收记录；会话级变更文件归因/diff 视图/S0-S3 独立页随后续阶段），验收记录见 specs/2026-07-18-p1f-acceptance.md。首个 dogfooding 交付即在 Dozer 内完成并经此流程验收。**
...
6. **对话可审阅性**：以人类发言为导航锚；AI 回合过程性输出默认折叠；提问/待决策/交付声明置顶；产物优先于叙述（看 diff/预览而非自述）。与验收闭环同构：作品级 vs 回合级。
   - _达成（2026-07-20，P1i+P1j 合一验收）_：P1i 落审阅引擎（transcript JSONL 适配器 + 左二审阅 tab：人类锚点 ▎+ AI 回合折叠、▸ 展开过程）；P1j 落入口（右一 AI 栏"对话"列表，扫 Claude 目录、活对话置顶）。"提问置顶(c)/产物联动(d)"推后续；三层分组(agent▸session▸conversation)待 §8 索引。
...
| 栏 | 域 | 视图切换 |
|----|----|---------|
| 左一 项目栏 | 项目域 | 项目卡 + 单视图内容区（文件树 / git / 组件，**底部 tab 切换**——真实项目文件多，不混排）；git 段常显当前分支与脏标记（`main*`）；组件=配置文件的可视化预览（数据库、模型运行状态）；底部 doctor。顶部视图位留给任务等项目管理级视图（二期）。**P1g 达成（2026-07-19，最小可用项目层：项目对象/持久化/当前+最近项目、项目卡含 git 分支脏、懒加载文件树、点文件进预览、重锚终端与验收到当前项目、切换清理；组件视图/doctor/文件树 git 装饰/H0 独立页随后续。项目存 dozerd SQLite），验收记录见 specs/2026-07-19-p1g-acceptance.md。P1h 达成（2026-07-19，文件树 git 装饰：金(改)/绿(新)/红(删)上色 + 尾缀字符 + 目录聚合 rollup + 打开/回合结束刷新），验收记录见 specs/2026-07-19-p1h-acceptance.md** |
...
| 左四 AI 栏 | AI 域 | ~~不含会话列表~~ **[2026-07-20 P1j 修订]** 顶部视图切换 `[对话\|Agents]`,默认"对话":列当前项目历史对话(活对话置顶),点进左二审阅——会话是"每时每刻"关注点,置于 Agents 前;Agents 简卡(名称+状态+用量)次之。Memory(项目级、跨 agent,三期)与编排(多路对照/派单,二期)为带分期角标的折叠占位行 |

跨域元素：顶部居中全局搜索（⌘K）、标题栏目标锚（当前迭代目标常驻）、右上角设置入口。无底部全局状态栏（刻意留白，等有重要信息再启用；agent 用量配额显示在 Agent 卡片内）。

**P1k 达成（2026-07-27，外壳 chrome 还原度：整体气质接近 Figma S1、协调可用）**：①顶栏落地——Dozer 标题 + ⌘K 搜索框（视觉占位，检索排后续）+ 金色目标胶囊（读 `.dozer/goal.md` 首行）+ 设置齿轮（占位）；`view()` 改为 `column![top_bar, row![四栏]]`。②状态条落地——项目栏底"环境/dozerd + [文件\|git\|组件]"、终端栏底"agent 态 · resume · dozerd 持有"（**分栏底部条，非单一全局底栏**，仍守"无底部全局状态栏"）。③文件树字形（▾/▸/·）+ 行尾彩色状态点（金改/绿新/红删）。④卡片/pill 圆角打磨 + 项目卡"N 次验收"金副行（按 repo 查 dozerd 验收库）。搜索检索/设置面板/Context% 计量/agent 用量遥测以视觉占位或省略立骨架，接线各自排后续。验收记录见 specs/2026-07-20-p1k-acceptance.md。

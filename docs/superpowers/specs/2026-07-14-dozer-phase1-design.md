...
2. **内建产物预览**（体验支柱，与终端并列）：Markdown、图片、PDF、代码/diff、Office 等在应用内直接看；"不能让用户总跳出 Dozer，这是混乱的来源"；从对话、文件树、验收视图处处一键可达。
...
- **头号技术风险**：iced(wgpu) × 原生 WebView 的视图合成（焦点/滚动/z-order），第一周 spike 验证；降级方案（预览独立窗口）仅作最后手段。
...
| 左二 资产预览 | 项目资产的预览与验收现场 | 一切资产皆可预览成 tab：文件（Flyfish 渲染）、diff、**会话审阅**（对话史=过程性资产，结构化预览：交付卡+人类锚点+折叠 AI 回合）。状态条含"AI x 分钟前修改" |
...
- H0 社区板块的内容源（教程文章/视频托管在哪、如何更新）—— 实现时可先静态占位。
- H0 日历的数据源——一期仅从项目事件（验收、目标）派生展示；"例行任务"类调度是后续能力。

...
/// 内容里有 `hover_t` 缓动动画的弹层,`HOVER_ANIM_INTERVAL`(`main.rs`,
/// ~60fps)会在过渡期间持续触发整棵子树重绘,噪点层的额外开销被按帧数
/// 放大——2026-09-13 用户反馈那里 hover 明显卡顿,release 构建下依然卡,
/// 排除是 debug 构建噪音,定位到就是这份放大。因此那个调用点保留不带
/// 噪点的 `shell()`,其余静态弹层改用这个。
